// Procedural City v5, final version (stage 5 of 5)
// A seeded, rule-based generator of isometric cities: irregular streets
// with an avenue, an optional river with bridges, density-driven zoning,
// a landmark tower and three lighting modes.
// Controls: click / [R] new city, [N] day-sunset-night, [1][2][3] grid
// size, [S] save PNG, [H] hide UI.

const GRID_SIZES = { '1': 16, '2': 22, '3': 28 };
...
// density field -> zoning -> variety pass. One seed, one city.
  ...
  for (let i = 0; i < gridSize; i++) {
    ...
    for (let j = 0; j < gridSize; j++) {
      ...
      if (water[i][j] && road) {
        ...
        } else if (nearWat && random() < 0.6) cell.t = 'park';
        else if (noise(i * 0.3 + 99, j * 0.3 + 99) > 0.52) cell.t = 'park';

        if ((cell.t === 'tower' || cell.t === 'mid') &&
            roadNeighbors(i, j) >= 2 && d > 0.6) cell.f += 2;
        if (cell.t === 'tower') cell.f = min(cell.f, 15);
        if (cell.t === 'mid')   cell.f = min(cell.f, 8);
...
// Guarantees that every zone type exists and exactly one landmark tower.
function ensureVariety() {
  const all = [];
  for (let i = 0; i < gridSize; i++)
    for (let j = 0; j < gridSize; j++) {
      const c = city[i][j];
      if (c.d !== undefined) all.push(c);
    }
  const of = t => all.filter(c => c.t === t);
  const byD = a => a.slice().sort((x, y) => y.d - x.d);

  if (!of('tower').length && all.length) {
    const c = byD(all)[0]; c.t = 'tower'; c.f = 8 + floor(c.r1 * 5);
  }
  if (!of('mid').length && all.length) {
    const c = byD(all)[1] || byD(all)[0];
    if (c.t !== 'tower') { c.t = 'mid'; c.f = 4; }
  }
  if (!of('house').length && all.length) {
    const c = byD(all)[byD(all).length - 1];
    if (c.t === 'grass' || c.t === 'park') { c.t = 'house'; c.f = 1; }
  }
  if (!of('park').length) {
    const g = of('grass');
    if (g.length) {
      const c = byD(g)[g.length - 1];
      c.t = 'park'; c.trees = [tree(), tree(), tree()];
    }
  }
  const towers = byD(of('tower'));
  if (towers.length) {
    towers[0].f = min(towers[0].f + 4, 17);
    towers[0].landmark = true;
  }
}
...
function roofUnit(g, cx, cyc, bw, hpx, base, c) {
  ...
  if (c.landmark) {
    const ax = cx, ay = ty - uH, aTop = ay - TW * 0.55;
    g.stroke(light('#3a3f4a')); g.strokeWeight(2);
    g.line(ax, ay, ax, aTop);
    g.noStroke();
    landmarkPos = { x: OX + ax, y: OY + aTop };
  }
}
...
function drawBeacon() {
  if (!landmarkPos) return;
  const b = (sin(frameCount * 0.1) + 1) / 2;
  noStroke();
  fill(255, 70, 70, 40 + b * 60);
  circle(landmarkPos.x, landmarkPos.y, 16 + b * 10);
  fill(255, 80, 80, 130 + b * 125);
  circle(landmarkPos.x, landmarkPos.y, 5);
}

function drawHUD() {
  const l1 = 'SEED ' + nf(seed, 5) + '   ' + MODES[mode] +
             '   ' + gridSize + '\u00d7' + gridSize;
  const l2 = '[CLICK/R] new city  [N] light  [1 2 3] size  ' +
             '[S] save  [H] hide UI';
...
function keyPressed() {
  ...
  if (GRID_SIZES[k]) { gridSize = GRID_SIZES[k]; regenerate(); }
  if (k === 's') saveCanvas('procedural_city_' + seed, 'png');
  if (k === 'h') hideUI = !hideUI;
}
